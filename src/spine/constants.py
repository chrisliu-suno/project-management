...
TICKET_KEY_PATTERN = r"\b([A-Z]{2,6})-\d+\b"
...
SENTENCE_END_CHARACTERS = frozenset(".!?")
PURPOSE_FIELD_PREFIXES = ("**Purpose:**", "Purpose:")
...
DOCS_INDEX_ACTION = "index"
DOCS_OUT_OPTION = "--out"
DOCS_OUT_DEST = "out_path"
DOCS_ROOT_OPTION = "--root"
DOCS_ROOT_DEST = "root_dir"
DOCS_UNSTATED_ROW_FORMAT = "{path}\tno stated purpose\t{brief}"
DOCS_UNSTATED_SUMMARY_FORMAT = (
    "{unstated} of {total} documents state no purpose; "
    "name them in a `| Read | For |` table in their corpus's start-here document"
)
DOCS_DERIVED_BRIEF_PLACEHOLDER = "(guessed from the body)"
DOCS_README_WRITTEN_FORMAT = "wrote {path}: {documents} documents across {projects} projects"

README_HEADER = "# Docs"
README_GENERATED_NOTICE = (
    "Generated by `spine docs index`. Every line is the purpose a document's corpus states for "
    "it, so edit the corpus's start-here table rather than this page."
)
README_PROJECT_HEADING_PREFIX = "## "
README_SECTION_SEPARATOR = "\n\n"
README_DOC_LINE_TEMPLATE = "- [`{name}`]({path}) — {brief}"
README_UNBRIEFED_LINE_TEMPLATE = "- [`{name}`]({path})"
...
MAX_PHRASING_SENTENCE_CHARS = 300
ONBOARDING_STALE_ENTRY_DAYS = 30
ONBOARDING_MAX_REPORTED_DOCS = 8
PLAN_HISTORY_DB_FILE_NAME = "plan_history.sqlite3"
PLAN_HISTORY_MAX_POINTS = 60
